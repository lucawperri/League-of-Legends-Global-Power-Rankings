...
def download_games(year, target_data_size_bytes):
    ...
    total_data_size_bytes = 0  # Track the total downloaded data size
                    ...
                    for match in section["matches"]:
                        for game in match["games"]:
                            if game["state"] == "completed":
                                ...
                                # Download and track the data size
                                data_size_bytes = download_gzip_and_write_to_json(f"{directory}/{platform_game_id}")
                                total_data_size_bytes += data_size_bytes

                                if total_data_size_bytes >= target_data_size_bytes:
                                    print(f"Downloaded {total_data_size_bytes / (1024**3):.2f} GB of data.")
                                    return  # Exit the loop when the target size is reached

                            if total_data_size_bytes % (1024**3) == 0:
                                print(
                                    f"Downloaded {total_data_size_bytes / (1024**3):.2f} GB of data, current run time: \
                                    {round((time.time() - start_time) / 60, 2)} minutes"
                                )

if __name__ == "__main__":
    download_esports_files()
    # Specify the target data size in bytes (20 GB)
    target_data_size_bytes = 20 * 1024**3  # 20 GB in bytes
    download_games(2023, target_data_size_bytes)